...
//        let personal = Company(name: "Personal Projects")
        ...
        let everlivProject = try! Project.Builder()
            ...
            .addDescription("Developed OpenAPI YAML specs defining REST and Server-Sent Events (SSE) endpoints for real-time chat functionality")
            .addDescription("Built a mock Swift Vapor server with REST API and SSE streaming for development and testing")
            .addDescription("Integrated Swift OpenAPI Generator to produce type-safe server and modular Swift networking code")
            .addDescription("Created cross-platform SwiftUI chat clients for macOS and iOS with real-time messaging and responsive UIs")
            .addDescription("Engineered a custom SSE client with async/await handling, error resilience, and fine-grained streaming control")
            .addDescription("Refactored chat architecture with modular components, adaptive UI, and scalable patterns")
            .addDescription("Implemented an ActionCard system supporting rich, expandable content types with optimized UX")
            .addDescription("Enhanced user experience with image uploads, previews, and cross-platform interaction consistency")
            .addDescription("Maintained steady development cadence with iterative improvements, testing, and documentation")
